    ...
    <div class="product-card group cursor-pointer h-full flex flex-col">
      ...
      <div class="relative overflow-hidden bg-gray-50 aspect-square">
        ...
        @if (product.discount && product.discount > 0) {
          <div class="absolute top-2 left-2 bg-flipkart-green text-white text-xs font-semibold px-2 py-1 rounded">
      ...
      <div class="flex-1 p-3 flex flex-col">
        ...
        <h3 class="text-sm font-medium text-gray-900 mb-2 line-clamp-2 flex-1">
          <a [routerLink]="['/product', product.id]" class="hover:text-flipkart-blue transition-colors">
            {{ product.title }}
          </a>
        </h3>
        ...
        @if (product.tags && product.tags.length > 0) {
          <div class="flex flex-wrap gap-1 mb-3">
            @for (tag of product.tags.slice(0, 2); track tag) {
              <span class="text-xs bg-flipkart-blue text-white px-2 py-1 rounded">
                {{ tag }}
              </span>
            }
          </div>
        ...
        @if (product.freeDelivery) {
          <div class="flex items-center text-xs text-flipkart-green font-medium mb-3">
...
export class ProductCardComponent {
  ...
  toggleWishlist(event: Event) {
    ...
    if (this.isInWishlist()) {
      this.wishlistItems.set(currentWishlist.filter(id => id !== this.product.id));
