...
import { LocationService, DeliveryLocation } from '../../services/location.service';
  ...
  template: `
    <div class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
      <div class="bg-white rounded-lg shadow-xl max-w-md w-full max-h-[90vh] overflow-hidden">
        <!-- Header -->
        <div class="bg-flipkart-blue text-white p-4 flex items-center justify-between">
        ...
        <div class="p-4 overflow-y-auto max-h-[calc(90vh-80px)]">
          ...
          <!-- Popular Cities -->
          <div>
            <h4 class="text-sm font-medium text-gray-700 mb-3">Popular Cities</h4>
            <div class="grid grid-cols-1 gap-2">
              @for (city of popularCities; track city.pincode) {
                <button
                  ...
                  [class.border-flipkart-blue]="city.pincode === locationService.location().pincode"
                  [class.bg-blue-50]="city.pincode === locationService.location().pincode"
                >
                  <div class="font-medium text-gray-900">{{ city.city }}</div>
                  <div class="text-sm text-gray-500">{{ city.state }} - {{ city.pincode }}</div>
                </button>
              }
            </div>
          </div>
...
export class LocationPickerComponent {
  ...
  detectLocation(): void {
    ...
    this.locationService.getCurrentLocation().subscribe({
      ...
      error: () => {
        this.detecting.set(false);
        this.pincodeError.set('Could not detect location. Please select manually.');
      }
    });
  }
